...
import cs.min2phase.SearchKoc;

public class testKoc {
    ...
    static int testOptimal(int maxl, int length, int[] arr, int lm, SearchKoc search, int verbose) {
        if (maxl == 0) {
            String sol = search.solution(Tools.fromScramble(arr), 100, 0, 0, verbose | SearchKoc.INVERSE_SOLUTION);
            long nProbe = 0;
            while (sol.startsWith("Error") || search.length() > length) {
                ...
                sol = search.next(PROBES_PER_NEXT, 0, verbose | SearchKoc.INVERSE_SOLUTION);
    ...
    static void testRandomOptimal(int length, SearchKoc search, Random gen, int verbose) {
    ...
    static class solvingThread extends Thread {
        ...
        int probeMin;
        int verbose;
        SearchKoc search = new SearchKoc();
    ...
    static void selectedScrambleSolvingTest(boolean isOptimal, SearchKoc search, int maxDepth) {
        System.out.println(
            isOptimal
        ...
        System.out.println(String.format("IdCube Test: \"%s\"", search.solution(scr, 21, 100000, 0, SearchKoc.OPTIMAL_SOLUTION)));
        ...
        for (int length = 1; length <= MAX_OPT_ALL; length++) {
            ...
            cumNProbe = 0;
            maxNProbe = 0;
            n_test = testOptimal(length, 0, new int[length], -1, search, isOptimal ? SearchKoc.OPTIMAL_SOLUTION : 0);
        ...
        for (int length = MAX_OPT_ALL + 1; length <= maxDepth; length++) {
            ...
            while (System.nanoTime() - curTime < MAX_TEST_TIME) {
                ...
                testRandomOptimal(length, search, gen, isOptimal ? SearchKoc.OPTIMAL_SOLUTION : 0);
                ++n_test;
            }
            System.out.println(String.format("OK, All %d Cube(s) Solved Optimally. AvgTime: %1.3f ms. TotP: %d. MaxP: %d", n_test, (System.nanoTime() - curTime) / 1000000d / n_test, cumNProbe, maxNProbe));
        }
    ...
    public static void main(String[] args) {
        ...
        if ((testValue & 0x10) != 0) {
            ...
            for (int i = 0; i < depth20.length; i++) {
                sol = search.solution(Tools.fromScramble(depth20[i]), 20, 100000, 0, SearchKoc.INVERSE_SOLUTION);
